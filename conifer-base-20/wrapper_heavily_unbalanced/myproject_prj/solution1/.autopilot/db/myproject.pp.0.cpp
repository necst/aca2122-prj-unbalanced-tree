...
#pragma HLS unroll
 ...
 if(i == 0){
  ...
  score_t y = 0;
  VITIS_LOOP_147_1: for(int i = 0; i < n_leaves; i++){
...
public:
 ...
 void decision_function(input_t x, score_t score[fn_classes(n_classes)], score_t tree_scores[fn_classes(n_classes) * n_trees]) const{
  ...
  VITIS_LOOP_188_1: for(int j = 0; j < fn_classes(n_classes); j++){

...
void print_axi_result(res_T result[SIZE], std::ostream &out, bool keep = false) {
    for(int i = 0; i < SIZE; i++) {
        out << "{ data: " << result[i].data << ", last: " << result[i].last << " }" << std::endl;
    }
    out << std::endl;
}

template<class res_T, size_t SIZE>
void print_axis_result(res_T result, std::ostream &out, bool keep = false) {
	out << "{ data: ";
    for(int i = 0; i < SIZE; i++) {
        out << result.data[i] << ", ";
    }
    out << ", last: " << result.last << "}" << std::endl;
}
